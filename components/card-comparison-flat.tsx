...
export function CardComparison({ card1, card2 }: CardComparisonProps) {
  ...
  const calc = (card: CreditCardData) => {
    // Multipliers are stored as decimals in DB (0.05 = 5x)
    // Convert to display rate by multiplying by 100
    const groceryRate = card.groceryMultiplier * 100
    const gasRate = card.gasMultiplier * 100
    const diningRate = card.diningMultiplier * 100
    const billsRate = card.billsMultiplier * 100
    
    // Calculate points earned per category (annual)
    const groceryPoints = spending.grocery * 12 * groceryRate
    const gasPoints = spending.gas * 12 * gasRate
    const diningPoints = spending.dining * 12 * diningRate
    const billsPoints = spending.bills * 12 * billsRate
    
    // Convert points to dollar value (1 point = $0.01)
    const g = groceryPoints / 100
    const f = gasPoints / 100
    const d = diningPoints / 100
    const b = billsPoints / 100
  ...
  const metrics = [
    ...
    },
    {
      ...
      v1: formatRewardRate(card1.baseRewardRate),
      v2: formatRewardRate(card2.baseRewardRate),
    ...
    },
    {
      ...
      v1: formatRewardRate(card1.groceryMultiplier),
      v2: formatRewardRate(card2.groceryMultiplier),
    ...
    },
    {
      ...
      v1: formatRewardRate(card1.gasMultiplier),
      v2: formatRewardRate(card2.gasMultiplier),
    ...
    },
    {
      ...
      v1: formatRewardRate(card1.diningMultiplier),
      v2: formatRewardRate(card2.diningMultiplier),
    ...
    },
    {
      ...
      v1: formatRewardRate(card1.billsMultiplier),
      v2: formatRewardRate(card2.billsMultiplier),
    ...
    <div className="container mx-auto px-4 pb-24 md:pb-12 space-y-6 max-w-5xl">
      ...
      <Card className="glass-premium border-primary/20">
        <CardContent className="p-6 md:p-8">
          ...
          <div className="rounded-xl relative overflow-hidden border border-purple-400/40 p-4 bg-gradient-to-br from-purple-900/20 via-indigo-900/20 to-cyan-900/20">
            ...
            <div className="relative z-10 text-center">
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                <span className="text-sm font-semibold text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-pink-300 to-cyan-300 uppercase tracking-wide">Best Value</span>
              </div>
              <p className="font-bold text-lg md:text-xl">
                {winner.name} wins by <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-200 via-pink-200 to-cyan-200">${diff}</span>
              </p>
      ...
      {/* ── Spending panel (desktop) / collapsible (mobile) ── */}
      <div>
